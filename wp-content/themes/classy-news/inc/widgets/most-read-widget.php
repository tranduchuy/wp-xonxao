<?php
if ( ! class_exists( 'Classy_News_Most_Read_Widget' ) ) {
	/**
	 * Adds Classy News Most Read Widget.
	 */
	class Classy_News_Most_Read_Widget extends WP_Widget {
		...
		public function __construct() {
			$most_read_widget = array(
				...
				'description' => __( 'Retrive Most Read Widgets', 'classy-news' ),
			);
			parent::__construct(
				'classy_news_most_read_widget',
				__( 'Adore Widget: Most Read Widget', 'classy-news' ),
				$most_read_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$most_read_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 6;
			...
			if ( ! empty( $most_read_title || $viewall_button_label ) ) {
				...
				<div class="widget-header">
					<?php echo $args['before_title'] . esc_html( $most_read_title ) . $args['after_title']; ?>
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show (Min: 1 | Max: 8):', 'classy-news' ); ?></label>
